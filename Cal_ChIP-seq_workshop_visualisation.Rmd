...
There are several differet methods available for normalisation, this calibration workflow requires Reads Per Million (RPM), so we need to extract the number of mapped reads from the flagstat report, calculate the RPM value and use it as an argument when creating out track. Normally this process isn't as complex and we'd use a single command to do this.

<div class="blue">
```bash
# Create BigWigs (READS PER MILLION)
# Get the number of mapped reads from flagstat
num1=`cat  mm2_out/WT_IP.experimental.only.mm2.rDNA.bam.fs | grep N/A|grep mapped | grep N/A |cut -f1 -d ' '`
echo "$num1"
...
bedtools genomecov -ibam  mm2_out/WT_IP.experimental.only.mm2.rDNA.bam -bg -scale $RPM  > mm2_out/WT_IP.experimental.only.rDNA.rpm.bedgraph

# Convert to bigwig
wigToBigWig mm2_out/WT_IP.experimental.only.rDNA.rpm.bedgraph /homes/genomes/s.cerevisiae/sacCer3/sacCer3.len  mm2_out/WT_IP.experimental.only.rDNA.rpm.bw
```
</div>
...
```bash
# clean up pombe
rm mm2_out/WT_IP.calibration.mm2.sam
rm mm2_out/WT_IP.calibration.mm2.unmapped.bam 

# clean up saccer3
rm mm2_out/WT_IP.experimental.only.mm2.sam
```
</div>

<br>

<img src="images/diag_2.png" width=1200>

So far we have mapped our chIP sample to pombe and then sacCer genomes, this has been filtered and normalized to Reads Per Million and is ready for visualisation, but before we can calibrate this sample we need to map it to sacCer and then pombe. We also need to go through the whole process for our Input sample as well. Before moving onto IGV lets move ahead to the next section and start our workflow.
...
```bash
mkdir visualisation
ln -s $PWD/mm2_out/*bw visualisation
ln -s $PWD/mm2_out/*rDNA.bam* visualisation
```
</div>
